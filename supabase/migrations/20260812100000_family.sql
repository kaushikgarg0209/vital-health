-- ─────────────────────────────────────────────────────────────────────────────
-- Phase 8 — Family Health Network
-- ─────────────────────────────────────────────────────────────────────────────
--
-- LEARNING OVERVIEW:
--
-- 1. ENUMS       — Permission levels and membership lifecycle states
-- 2. TABLES      — family_groups (container), family_memberships (directed grants),
--                  notifications (in-app alerts for caregivers)
-- 3. PROFILE ALT — Emergency contact fields for the emergency brief
-- 4. HELPERS     — SQL functions so RLS policies stay readable
-- 5. RLS         — Caregivers can SELECT shared data; owners keep full control
--
-- Data model: family_memberships is a DIRECTED grant:
--   subject_user_id = whose health data is shared (e.g. Rahul)
--   viewer_user_id  = who can view it (e.g. Meera), NULL while invite is pending
-- ─────────────────────────────────────────────────────────────────────────────
